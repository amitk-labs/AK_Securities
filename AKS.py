...
if "win" in os_name: # if operating system is Windows
    if not os.path.exists("AKS\\Quarantine\\"):
        os.makedirs("AKS\\Quarantine\\") # Directory for Quarantine files
    if not os.path.exists("AKS\\sf\\"):
        os.makedirs("AKS\\sf\\") # Partition folder
    if not os.path.exists("AKS\\Large_Update_File\\"):
        os.makedirs("AKS\\Large_Update_File") # Directory for program update files
    quarantine_folder = "AKS\\Quarantine\\*"
    file_to_quarantine = "AKS\\Quarantine\\"
    partition_folder = "AKS\\sf\\sf.txt"
    links_current = "AKS\\Large_Update_File\\links_current.txt"
    links_downloaded = "AKS\\Large_Update_File\\links_downloaded.txt"
    large_signatures = "AKS\\Large_Update_File\\signatures.txt"
    f = open(partition_folder, "a")
    f.close()
    ...
    if not os.path.exists("AKS//Quarantine//"):
        os.makedirs("AKS//Quarantine//")
    if not os.path.exists("AKS//sf//"):
        os.makedirs("AKS//sf//")
    if not os.path.exists("AKS//Large_Update_File//"):
        os.makedirs("AKS//Large_Update_File//")
    quarantine_folder = "AKS//Quarantine//*"
    file_to_quarantine = "AKS//Quarantine//"
    partition_folder = "AKS//sf//sf.txt"
    links_current = "AKS//Large_Update_File//links_current.txt"
    links_downloaded = "AKS//Large_Update_File//links_downloaded.txt"
    large_signatures = "AKS//arge_Update_File//signatures.txt"
    f = open(partition_folder, "a")
    f.close()
    f = open(links_current, "a")
    f.close()
...
if daytime >= 18 or daytime <= 4:
    ...
    fgc = "black"
    special = "#1ccaed"
    special_text = "\(≧∇≦)/ Welcome " + os.getlogin() + " to AKS \(≧∇≦)/\n"
...
def ScanSystemFiles():
    ...
    text_box.insert(END, "[ * ] Scanning the system for files...\n")
    text_box.see(END)
    text_box.update()
    time.sleep(3)
    text_box.see(END)
    text_box.update()
    SystemFileScanner.partitions(partition_folder)
    f = open(partition_folder, "r")
    content = f.read()
    f.close()
...
def full_scan(part):
    ...
    while i >= tmp:
        ...
        if ret == True:
            ...
            text_box.see(END)
            text_box.update()
            quarantine_lib.encode_base64(files[i])
        files_len -= 1
        i -= 1
    ...
    if len(terminations) == 0:
        text_box.insert(END, "[ +++ ] Your PC is safe, enjoy" + "\n", 'important')
    else:
        text_box.insert(
...
def restore(file, ALL):
    ...
    if len(terminations) != 0:
        if ALL == 1:
            for i in range(len(terminations)):
                quarantine_lib.decode_base64(terminations[i])
                text_box.insert(
            ...
            quarantine_lib.decode_base64(file)
            li.delete(ACTIVE, len(file))
            text_box.insert(
                END, "[ + ] Successfully restored\n" + file + "\n", "positive")
            text_box.tag_config("positive", foreground="green")
...
def add_file_to_quarantine():
    ...
    file = file.replace("/", "\\")
    quarantine_lib.encode_base64(file, file_to_quarantine)
...
def scan():
    ...
    try:
        ...
        text_box.insert(
            END, "[ - ] Something went wrong while performing the task\n", "negative")
        text_box.tag_config("negative", foreground="red")
        text_box.see(END)
        text_box.update()
        return None
    ...
    if match:
        ...
        text_box.update()
    if not match:
        text_box.insert(END, "[ + ] No threat was found, you are safe!\n", "positive")
        text_box.tag_config("positive", foreground="green")
        text_box.see(END)
        text_box.update()
...
def link_collector():  # gets Links to refresh update-site;short spider
    ...
    u = urllib.request.urlopen(
        "http://virusshare.com/hashes").read().decode("utf-8").splitlines()
...
def button_action_handler(s):
    ...
    if s == "fullscan_button":
        if files_len == 0:
            text_box.insert(END, "[ ! ] Preparing AKS full scan\n", "important")
            text_box.see(END)
            text_box.update()
...
def gui_thread():
    ...
    fullscan_button = tkinter.Button(main, bg=bgc, fg=fgc, text="Full scan", command=lambda: button_action_handler(
        "fullscan_button"), height=hoehe, width=breite)
    ...
    quarantine_button = tkinter.Button(main, bg=bgc, fg=fgc, text="Quarantine", command=lambda: button_action_handler(
        "quarantine_button"), height=hoehe, width=breite)
    quarantine_button.grid(row=3, column=0)
    quit_button = tkinter.Button(main, bg=bgc, fg=fgc, text="Close", command=lambda: button_action_handler(
        "quit_button"), height=hoehe, width=breite)
    ...
    text_box.insert(END, "[ + ] AKS is getting ready at your service..\n", 'positive')
    text_box.tag_config('positive', foreground='green')
    text_box.see(END)
    text_box.update()
    text_box.insert(
        END, "[ ! ] Give me a moment please ;) \n", 'important')
    text_box.tag_config('important', foreground="red")
    text_box.see(END)
    text_box.update()
    #row_counter += 3
...
t_files = Thread(target=ScanSystemFiles) # Thread for scanning system files
t_clock = Thread(target=clock_thread) # Thread for clocking
t_main.start()
time.sleep(1)
t_clock.start()
time.sleep(5)
